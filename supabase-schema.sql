...
create table if not exists public.profiles (id uuid primary key references auth.users(id) on delete cascade, name text not null default '', email text not null default 'RH' check (role in ('ADMIN','RH','DIRECCION','RESPONDENT')), active boolean not null default true, created_at timestamptz not null default now());
alter table public.profiles drop constraint if exists profiles_role_check;
alter table public.profiles add constraint profiles_role_check check (role in ('ADMIN','RH','DIRECCION','RESPONDENT'));
...
create or replace function public.can_view_studies() returns boolean language sql stable security definer set search_path = public as $$ select exists (select 1 from public.profiles where id = auth.uid() and role in ('ADMIN','RH','DIRECCION') and active = true); $$;
...
drop policy if exists "RH y admins ven estudios" on public.estudios_registros; drop policy if exists "Encuestado ve su estudio" on public.estudios_registros; drop policy if exists "RH o encuestado crea estudio" on public.estudios_registros; drop policy if exists "RH o encuestado actualiza estudio" on public.estudios_registros; drop policy if exists "RH y administradores eliminan estudios" on public.estudios_registros;
...
create policy "RH y admins ven estudios" on public.estudios_registros for select to authenticated using (public.can_view_studies());
...
create policy "RH y administradores eliminan estudios" on public.estudios_registros for delete to authenticated using (public.is_hr_or_admin());
