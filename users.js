...
module.exports = async (req, res) => {
  ...
  if (req.method === 'POST') {
    ...
    if (error) return res.status(400).json({ error: error.message });
    const assignedRole = ['ADMIN', 'RH', 'DIRECCION', 'RESPONDENT'].includes(role) ? role : 'RH';
    const { error: profileError } = await service.from('profiles').upsert({ id: data.user.id, name, email, role: assignedRole, active: true });
    if (profileError) {
      await service.auth.admin.deleteUser(data.user.id);
      return res.status(400).json({ error: `No se pudo asignar el rol: ${profileError.message}` });
    }
  ...
  if (req.method === 'PATCH') {
    ...
    if (role) {
      const assignedRole = ['ADMIN', 'RH', 'DIRECCION', 'RESPONDENT'].includes(role) ? role : null;
      if (!assignedRole) return res.status(400).json({ error: 'Rol no válido.' });
      const { error } = await service.from('profiles').update({ role: assignedRole }).eq('id', userId);
  ...
  if (req.method === 'DELETE') {
    ...
    if (!userId || userId === current.id) return res.status(400).json({ error: 'No puedes eliminar tu propia cuenta de administrador.' });
